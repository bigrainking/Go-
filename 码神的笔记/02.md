...
// 初始化所有的template：将他们解析完成可以用于直接填充数据
func InitTemplate(templateDir string) (HtmlTemplate,error)  {
	...
	htmlTemplate.Index = tp[0] // tp返回的是写好了的数组
...
func readTemplate(templates []string,templateDir string) ([]TemplateBlog,error)  {
	...
	for _,view := range templates{
		...
		header := templateDir + "layout/header.html"// 下面都是layout,这些都是templates都需要的（"index","category","custom","detail","login","pigeonhole","writing"）
